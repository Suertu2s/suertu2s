...
export async function confirmFlowPaymentByToken(
  ...
  if (Math.round(Number(flowStatus.amount)) !== order.total_clp) {
    ...
    const paidAmount =
      Number.isFinite(alt) && alt > 0 ? alt : Math.round(Number(flowStatus.amount));
    if (paidAmount !== order.total_clp) {
      logServerError(
        "payments/flow/confirm",
        new Error(
          `Monto Flow distinto pedido=${order.id} esperado=${order.total_clp} recibido=${paidAmount}`,
        ),
      );
